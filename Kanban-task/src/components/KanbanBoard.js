...
import { DndContext, DragOverlay, useSensor, useSensors, KeyboardSensor, TouchSensor, MouseSensor, } from "@dnd-kit/core";
import { SortableContext, arrayMove } from "@dnd-kit/sortable";
import { TaskCard } from "./TaskCard";
export function hasDraggableData(element) {
...
export function KanbanBoard() {
    ...
    return (_jsxs(DndContext, { accessibility: {
            announcements,
        }, sensors: sensors, onDragStart: onDragStart, onDragEnd: onDragEnd, onDragOver: onDragOver, children: [_jsx(BoardContainer, { children: _jsx(SortableContext, { items: columnsId, children: columns.map((col) => (_jsx(BoardColumn, { column: col, tasks: tasks.filter((task) => task.columnId === col.id) }, col.id))) }) }), "document" in window &&
                createPortal(_jsxs(DragOverlay, { children: [activeColumn && (_jsx(BoardColumn, { isOverlay: true, column: activeColumn, tasks: tasks.filter((task) => task.columnId === activeColumn.id) })), activeTask && _jsx(TaskCard, { task: activeTask, isOverlay: true })] }), document.body)] }));
